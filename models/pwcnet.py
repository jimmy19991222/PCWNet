...
class PWCNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if self.training:
            ...
            cost0 = F.upsample(cost0, [self.maxdisp, left.size()[2], left.size()[
                               3]], mode='trilinear', align_corners=True)
            cost0 = torch.squeeze(cost0, 1)
            pred0 = F.softmax(cost0, dim=1)
            pred0 = disparity_regression(pred0, self.maxdisp)

            cost1 = F.upsample(cost1, [self.maxdisp, left.size()[2], left.size()[
                               3]], mode='trilinear', align_corners=True)
            cost1 = torch.squeeze(cost1, 1)
            pred1 = F.softmax(cost1, dim=1)
            pred1 = disparity_regression(pred1, self.maxdisp)

            cost2 = F.upsample(cost2, [self.maxdisp, left.size()[2], left.size()[
                               3]], mode='trilinear', align_corners=True)
            cost2 = torch.squeeze(cost2, 1)
            pred2 = F.softmax(cost2, dim=1)
            pred2 = disparity_regression(pred2, self.maxdisp)

            cost3 = F.upsample(cost3, [self.maxdisp, left.size()[2], left.size()[
                               3]], mode='trilinear', align_corners=True)
            cost3 = torch.squeeze(cost3, 1)
            pred3 = F.softmax(cost3, dim=1)
            pred3 = disparity_regression(pred3, self.maxdisp)

            combine = F.upsample(combine, [self.maxdisp, left.size()[2], left.size()[
                                 3]], mode='trilinear', align_corners=True)
            ...
            refinenet_combine = torch.cat((refinenet_feature_left - refinenet_feature_right_warp,
                                          refinenet_feature_left, pred3feature, pred3, refinenet_costvolume), dim=1)
...
class PWCNet_wo_fine(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        super(PWCNet_wo_fine, self).__init__()
        self.maxdisp = maxdisp
        self.use_concat_volume = use_concat_volume
    ...
    def forward(self, left, right):
        ...
        if self.training:
            ...
            return [combine], [pred3]


def PWCNet_wo_fine_G(d):
    return PWCNet_wo_fine(d, use_concat_volume=False)


def PWCNet_wo_fine_GC(d):
    return PWCNet_wo_fine(d, use_concat_volume=True)
