...
parser = argparse.ArgumentParser(
    description='PCW-Net: Pyramid Combination and Warping Cost Volume for Stereo Matching')
parser.add_argument('--model', default='gwcnet-g',
...
parser.add_argument('--wo_fine', action='store_true',
                    help='without refine')
...
print('# pwcnet parameters:', sum(param.numel()
...
def train_sample(sample, compute_metrics=False):
    ...
    image_outputs = {"disp_est": disp_ests,
                     "disp_gt": disp_gt, "imgL": imgL, "imgR": imgR}
    if compute_metrics:
        with torch.no_grad():
            image_outputs["errormap"] = [disp_error_image_func().forward(
                disp_est, disp_gt) for disp_est in disp_ests]
            scalar_outputs["EPE"] = [EPE_metric(
                disp_est, disp_gt, mask) for disp_est in disp_ests]
            scalar_outputs["D1"] = [
                D1_metric(disp_est, disp_gt, mask) for disp_est in disp_ests]
            scalar_outputs["Thres1"] = [Thres_metric(
                disp_est, disp_gt, mask, 1.0) for disp_est in disp_ests]
            scalar_outputs["Thres2"] = [Thres_metric(
                disp_est, disp_gt, mask, 2.0) for disp_est in disp_ests]
            scalar_outputs["Thres3"] = [Thres_metric(
                disp_est, disp_gt, mask, 3.0) for disp_est in disp_ests]
    loss.backward()
    optimizer.step()
...
def test_sample(sample, compute_metrics=True):
    ...
    scalar_outputs["D1_pred3"] = [
        D1_metric(pred, disp_gt, mask) for pred in pred3]
